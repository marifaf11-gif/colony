...
import { runCyberchienScan, type CyberchienStep, type CyberchienResult } from '@/lib/pods/cyberhawk-engine';
...
const STEP_LABELS: Record<CyberchienStep, string> = {
  idle: '',
  dns: 'Résolution DNS & WHOIS...',
  ports: 'Balayage des ports ouverts...',
  headers: 'Audit des en-têtes HTTP...',
  tech: "Identification des technologies...",
  complete: 'Analyse terminée',
};

interface CyberchienClientProps {
  locale: Locale;
}

export function CyberchienClient({ locale }: CyberchienClientProps) {
  const [url, setUrl] = useState('');
  const [step, setStep] = useState<CyberchienStep>('idle');
  const [result, setResult] = useState<CyberchienResult | null>(null);
  ...
  const handleScan = async () => {
    ...
    try {
      const res = await runCyberchienScan(normalized, (s) => setStep(s));
      setResult(res);
    } catch {
      setError("Échec de l'analyse — vérifiez l'URL cible");
      setStep('idle');
    }
  };
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-white/35 mt-0.5">Talon HUD — Module de renseignement sécuritaire</p>
                </div>
              </div>
            </div>
            ...
            {isComplete && result && (
              ...
              >
                ...
                <span>{isGeneratingPdf ? 'Génération...' : "Rapport Ticket d'Or"}</span>
              </button>
            )}
          </div>
              ...
              >
                ...
                >
                  {isScanning ? (
                    ...
                    <>
                      ...
                      <span>{isComplete ? 'Nouvelle analyse' : 'Lancer l\'analyse Talon'}</span>
                    </>
                  )}
                </button>
                ...
                {isScanning && (
                  <div className="space-y-2">
                    {(['dns', 'ports', 'headers', 'tech'] as CyberchienStep[]).map((s) => {
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <div>
                      ...
                      >
                        Niveau de menace : {result.threatLevel}
                      </p>
                      <p className="text-xs text-white/40 mt-0.5">
                        {result.findings.length} trouvailles — Est. $
                        {result.totalRisk.toLocaleString('fr-CA')} exposition au risque
                      </p>
                    </div>
                  </div>
              ...
              {!isComplete && !isScanning && (
                ...
                >
                  ...
                  {[
                    ...
                    'Audit sécurité des en-têtes HTTP',
                    'Identification des technologies',
                    'Évaluation des risques CVE',
                    "Rapport PDF Ticket d'Or",
                  ].map((cap) => (
