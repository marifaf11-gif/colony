...
function generateGoldenTicketHtml(vuln: Record<string, unknown>, stripeLink: string): string {
  const palette = SEVERITY_PALETTE[vuln.severity as string] ?? SEVERITY_PALETTE.Medium;
  const impact = Number(vuln.impact_estimate ?? 0);
  const impactStr = `$${impact.toLocaleString("en-CA")}`;
  ...
  <body><div class="wrapper"><div class="header"><div style="display:flex;justify-content:space-between"><div><div class="logo">Colony OS // Strike Engine</div><div style="font-size:10px;color:rgba(255,255,255,.3)">Autonomous Revenue Recovery</div></div><div class="tid"><div>${escHtml(ticketId)}</div><div style="margin-top:4px">${escHtml(dateStr)}</div></div></div><div class="h1">Golden Ticket</div><div style="font-size:13px;color:rgba(255,255,255,.4);margin-bottom:12px">${escHtml(targetDomain)}</div><div class="severity-badge"><div class="dot"></div><span class="sev-label">${escHtml(String(vuln.severity))} Priority Kink Detected</span></div></div>
  ...
  <div style="font-size:10px;font-weight:700;letter-spacing:.25em;color:rgba(255,255,255,.2);text-transform:uppercase;margin-bottom:12px">Kink Discovered</div>
  <div class="kink"><div class="kt">${escHtml(String(vuln.title ?? ""))}</div><div class="kd">${escHtml(String(vuln.description ?? ""))}</div></div>
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    let kinks: Array<{ title: string; description: string; kink_type: string; severity: string; impact_estimate: number }> = [];

    if (openrouterKey) {
      const prompt = `You are The Hound. Analyze ${targetUrl} and return a JSON array of exactly 2 revenue kinks:
[{"title":"...","description":"...","kink_type":"vitals|conversion|seo|security","severity":"Critical|High|Medium|Low","impact_estimate":12345}]
Return ONLY the JSON array.`;
      ...
      if (aiRes.ok) {
        ...
        if (match) {
          try { kinks = JSON.parse(match[0]).slice(0, 2); } catch { /* noop */ }
        }
      }
    }
    ...
    const { data: vulnRow } = await supabase
      ...
      .insert({
        ...
        kink_type: primaryKink.kink_type,
        title: primaryKink.title,
        description: primaryKink.description,
        severity: primaryKink.severity,
        impact_estimate: primaryKink.impact_estimate,
        status: "pending",
        metadata: { source: "strike-engine-edge", kinks_count: kinks.length },
      })
      .select("id")
      .maybeSingle();
    ...
    await logTerminal(supabase, "HOUND", "VULN_FOUND", `VULN_FOUND → ${primaryKink.title} [${primaryKink.severity}]`, {
      target_url: targetUrl,
      vulnerability_id: vulnId,
      metadata: { kinks_found: kinks.length, severity: primaryKink.severity },
    });
    ...
    if (discordScoutWebhook) {
      await sendDiscordWebhook(discordScoutWebhook, {
        embeds: [{
          ...
          description: `**${primaryKink.title}**\n\`${targetUrl}\``,
          color: primaryKink.severity === "Critical" ? 0xff0000 : primaryKink.severity === "High" ? 0xff4500 : 0xffa500,
          fields: [
            { name: "Severity", value: primaryKink.severity, inline: true },
            { name: "Type", value: primaryKink.kink_type, inline: true },
            { name: "Impact", value: `$${primaryKink.impact_estimate.toLocaleString("en-CA")}`, inline: true },
          ],
    ...
    await supabase.from("vulnerability_log").update({ status: "hawk_processing" }).eq("id", vulnId);
    ...
    if (discordBountyWebhook && vulnFull) {
      const impactStr = `$${Number(vulnFull.impact_estimate).toLocaleString("en-CA")}`;
      discordMsgId = await sendDiscordWebhook(discordBountyWebhook, {
        content: `@here **HAWK: TARGET_ACQUIRED** — ${vulnFull.severity} kink on \`${targetUrl}\` — Est. ${impactStr}`,
        embeds: [{
          title: `BOUNTY STRIKE // ${vulnFull.severity} KINK DETECTED`,
          description: `**${vulnFull.title}**\n\n${vulnFull.description}`,
          ...
          fields: [
            ...
            { name: "Kink Type", value: String(vulnFull.kink_type), inline: true },
    ...
    return new Response(JSON.stringify({
      ...
      vulnerability_id: vulnId,
      ticket_id: ticket?.id,
      kinks_found: kinks.length,
      primary_kink: primaryKink.title,
      stripe_link: stripeLink,
